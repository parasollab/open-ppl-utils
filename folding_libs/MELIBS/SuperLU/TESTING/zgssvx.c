 ...
 *         Stype = NC or NR, Dtype = Z_, Mtype = GE. In the future,
 ...
 *         L has types: Stype = SC, Dtype = Z_, Mtype = TRLU.
 *
 ...
 *         X has types: Stype = DN, Dtype = Z_, Mtype = GE. 
	...
	*info = -9;
	...
	if (*info == 0) {
	    if ( lwork < -1 ) *info = -15;
		      ...
		      B->Stype != DN || B->Dtype != Z_ || 
		      B->Mtype != GE )
		*info = -16;
	    else if ( X->ncol < 0 || Xstore->lda < SUPERLU_MAX(0, A->nrow) ||
		      B->ncol != X->ncol || X->Stype != DN ||
		      X->Dtype != Z_ || X->Mtype != GE )
		*info = -17;
	}
    }
    if (*info != 0) {
