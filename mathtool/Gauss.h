...
namespace mathtool{
  ...
  bool hasNext = false;//flag that tells whether the next gaussian has been computed
  double gset; //saved random for consequtive call

  //generate a random number from N(_mean, _std).
  //generates 2 random numbers at a time, so it saves it for the next call
  inline double gaussRand(double _mean = 0, double _std = 1){
