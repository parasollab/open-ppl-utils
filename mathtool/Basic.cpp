...
namespace mathtool{
    ...
    */
    ...
    ///////////////////////////////////////////////////////////////////////
    // Add by Jyh-Ming Lien
    bool getDoubleValue(char * strTag, double * tmp, int size){
        if( strTag==NULL ) return false;

        int index=0;//tmp storage
        //extract values from string
        const char strDelimiter[]="[](), \t\n"; char * endPtr;
        char * token = strtok(strTag,strDelimiter);

        while( token!=NULL ){
            if( index==size ) break;
            tmp[index++]=strtod(token, &endPtr);
    ...
    bool getDoubleValue(char * strTag, double * tmp){
        if( strTag==NULL ) return false;
        char * endPtr;
        //extract values from string
        *tmp = strtod(strTag,&endPtr);
        return true;
    }
